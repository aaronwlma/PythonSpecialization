...
# Actual data: http://py4e-data.dr-chuck.net/comments_160222.html (Sum ends
# with 34)

# You do not need to save these files to your folder since your program will
# read the data directly from the URL. Note: Each student will have a
# distinct data url for the assignment - so only use your own data url for
# analysis.
...
# Request a URL and use BeautifulSoup to parse clean HTML data
url = input('Enter - ')
...
tags = parsedHtml('span')
for retrievedTag in tags:
    runningTotal = runningTotal + float(retrievedTag.contents[0])
    count = count + 1

# Print desired output
print('Count', count)
print('Sum', int(runningTotal))
